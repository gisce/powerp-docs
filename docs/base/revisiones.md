...
Això ho podrem fer a través del **Menú principal > Revisions Internes >
Configuració > Models**

.. image:: _static/menu_models.png

Per crear un nou model simplement cliquem a **Nou** com fem amb la resta de
formularis de l'ERP.

.. image:: _static/formulari_models.png

En el formulari tenim els següents camps:

* **Model**: A través d'aquest camp seleccionarem quin model és elq ue volem \
  configurar.
* **Actiu**: Ens servirà per marcar si un model està actiu o no per fer-li
  revisions. És útil per si un cop hem configurat un model i a partir d'un
  determinat moment no volem continuar tenint-lo, el podem desactivar i així no
  apereixarà més en el llistat. **Sempre s'ha de desectivar i mai eliminar**.
* **Nom de l'objecte**: És el nom descriptiu per aquest model dins de l'ERP.
* **Objecte**: És el nom intern d'aquest object.
* **Domini**: És una expressió que ens permetrà aplicar un filtre als registres
  que formen part d'aquest model. Veure :ref:`configuracio_de_dominis`.
* **SQL**: És un camp que ens mostrarà el domini transformat en SQL per tal si
  volem provar si els resultat és l'esperat.
...
* **Crear Menú**: Ens servirà per crear el menú per tal de poder accedir
  ...
  model.
  Per defecte crea el menú a **Menú > Revisions Internes > Revisions >
  Revisions de XXXXXX** on **XXXXXX** és el text del camp **Nom del menú** i en
...
Podrem configurar i veure les normatives a través de **Menú > Revisions
Internes > Configuració > Normatives**

.. image:: _static/menu_normatives.png
...
.. image:: _static/formulari_general_normativa.png

* **Normativa**: Nom i descripció de la normativa.
* **Model**: A quin model dels que hem configurat fa referència. Sempre s'ha de
  tenir configurat el model abans de configurar una normativa.
* **Activa**: Si la normativa està actiav o no. Sempre s'ha de desactivar i mai
...
* **Domini**: És una expressió que ens permetrà aplicar aquesta normativa a un
  ...
  que definirem en el domini. Veure :ref:`configuracio_de_dominis`.
* **SQL**: És un camp que ens mostrarà el domini transformat en SQL per tal si
  volem provar si els resultat és l'esperat.

.. note:: La dates s'utilitzen a l'hora d'assignar els ítems a revisar i es fa
          segons la data de la revisió.

Les normatives estan pensades per tal que sigui reaprofitables i es
poden crear normatives **concretes** i normatives més **generals**.

En el següent exemple només aplicarem la normativa als transformadors que la
marca sigui *JARA*, per això hem d'estabilir el domini.
...
.. image:: _static/guardat_normativa_general.png
...
.. image:: _static/formulari_items_normativa.png
...
.. image:: _static/formulari_items.png

En el formulari per la creació d'ítems i tenim els següents camps:

* **Codi**: Un codi que identificarà aquest item dins d'una revisió.
* **Tipus de camp**: En el cas que aquest ítem no sigui vàlid dins d'una revisió,
  ...
  * **Texte**: Quan l'ítem sigui incorrecte haurà d'introduïr un text.
...
.. image:: _static/guardat_items_form.png

I en la següent imatge podem veure el llistat de tots els ítems a revisar

.. image:: _static/guardat_items_list.png

-------------------------------
Configuració de les valoracions
-------------------------------

Les valoracions les podem trobar a través de **Menú > Revisions Internes >
Configuració > Valoracions**

.. image:: _static/menu_valoracions.png
...
.. image:: _static/formulari_valoracions.png
...
.. image:: _static/guardat_valoracions.png

També disposem del llistat de totes les valoracions que tenim configurades.

.. image:: _static/guardat_valoracions_tree.png
...
**Menú > Revisions Internes > Revisions > Revisions de XXXXXX** on **XXXXXX** és
...
* **Empresa**: Empresa que durà a terme la revisió, pot ser la pròpia un una
...
* **Núm. defectes reparats**: Un indicador del tipus *X/Y* on es mostra de tots
  els ítems que hi ha marcats com a incorrectes (*Y*), quants n'hi ha de reparats
  (*X*).

**Pestanya Observacions**

.. image:: _static/guardat_revisio_observacions.png

* **Observacions**: Camp de text per escriure observacions de la revisió.

**Pestanya Planificació**

.. image:: _static/guardat_revisio_planificacio.png
...
.. note::
   ...
   El camp **Núm. defectes reparats** només mostra informació vàlida quan l'estat de la revisió és
   **Tancada**.
...
.. image:: _static/formulari_revisio_item.png
...
* **Valoració**: En cas que l'ítem estigui marcat com a **Incorrecte** s'haurà
  ...
  afecte a la data límit de reparació d'aquest ítem.
...
* **Dades**: En el cas que l'ítem estigui configurat com a que s'ha d'entrar un
  texte ens apereixerà aquest camp.
...
* **Reparat**: Un camp per indicar si aquest ítem ha estat repart
...
per llistat quines revisions volem planificar a futur.

.. image:: _static/formulari_revisions_item_list.png

.. |image_limit_80| image:: _static/mostrar_limit_80.png
.. |image_canvi_limit_80| image:: _static/canvi_limit_80.png

.. note::

   Si tenim més de **80** registres, s'ha d'amliar el límit per tal de poder
   actuar amb tot el llistat a través de l'icona |image_limit_80| i després
   canviar el límit al valor necessari |image_canvi_limit_80|

Fem click al botó **Acció** de la barra superior.

.. image:: _static/boto_accio.png

Ens mostrarà l'assistent per planificar les noves revisions, mostrant la següent
pantalla.

.. image:: _static/planificacio_revisions.png
...
.. image:: _static/planificacio_revisions_list.png

--------
Concreta
--------

En la vista del formulari d'una revisió, al marge dret podem executar l'assistent
de planificacións futures.

.. image:: _static/planificacio_revisions_concreta.png
...
.. _configuracio_de_dominis:

=======================
Configuració de dominis
=======================

Els dominis es crean a partir d'una llista de tuples de tres elements. On el
...
Els operadors que tenim són els següents:
  ...
  * **like**: Té alguna coincidència amb el terce element (tenint en compte les
    majúscules) el caràcter comodí és el **%**. P.ex %jo% coincidiria amb joan,
    josep, ajornar, major, ...
  * **ilike**: El mateix que l'anterior però sense tenir en compte les majúscules.
  * **not like**: Cap coincidència.
  * **>**: Més gran que el terce element.
  * **>=**: Més gran o igual.
  * **<**: Més petit.
  * **<=**: Més petit o igual.

Es poden fer servir més d'una condició alhora

.. code-block:: python

   [('numero_fabricaio', '=', False), ('pes_total', '>', 200)]

Aquest domini retorna els que numero_fabricacio es NULL i el pes_total més gran
que 200.
...
* **& (AND)**: Operador per defecte, fa que es compleixin les condicions entre
  dos ANDs.
* **| (OR)**: Una condició o una altre
* **! (NOT)**: Nega tota una condició

Es fa servir `notació polonesa <http://ca.wikipedia.org/wiki/Notaci%C3%B3_polonesa>`_

Un exemple amb **OR** si volguessim els que pessen 100 o 200 seria:
